...
pres.title = "Globie - Sistema Basado en Conocimiento";
...
function sectionHeader(slide, title, subtitle) {
  ...
  slide.addText(title, {
    ...
    fontSize: 28, bold: true, color: C.deepBlue, fontFace: "Calibri", margin: 0,
  });
  if (subtitle) {
...
// ═══════════════════════════════════════════════════════════════════════════════
{
  ...
  s.addText("Sistema Basado en Conocimiento para Viajes", {
    x: 0.5, y: 2.8, w: 7, h: 0.6,
    fontSize: 18, color: C.mint, fontFace: "Calibri", italic: true, margin: 0,
  });
  s.addText("Chatbot de Inteligencia de Viajes con Visualización 3D", {
...
// ═══════════════════════════════════════════════════════════════════════════════
// DIAPOSITIVA 2 — Introducción al dominio
// ═══════════════════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  sectionHeader(s, "Introducción al Dominio", "Motivación y relevancia del proyecto");

  card(s, 0.3, 1.3, 4.4, 1.1, C.mint);
  s.addText("¿Qué es Globie?", { x: 0.55, y: 1.35, w: 4.0, h: 0.4, fontSize: 14, bold: true, color: C.deepBlue, fontFace: "Calibri", margin: 0 });
  s.addText("Asistente de viajes conversacional que responde preguntas en lenguaje natural sobre ciudades y países del mundo.", {
    x: 0.55, y: 1.72, w: 3.9, h: 0.65, fontSize: 11, color: C.textDark, fontFace: "Calibri", margin: 0,
  });

  card(s, 0.3, 2.55, 4.4, 1.15, C.teal);
  s.addText("¿Por qué es relevante?", { x: 0.55, y: 2.6, w: 4.0, h: 0.4, fontSize: 14, bold: true, color: C.deepBlue, fontFace: "Calibri", margin: 0 });
  s.addText("El turismo global genera 9,5 billones de $ anuales. Acceder a información fiable y comparativa sobre destinos es un reto real para viajeros y planificadores.", {
    x: 0.55, y: 3.0, w: 3.9, h: 0.65, fontSize: 11, color: C.textDark, fontFace: "Calibri", margin: 0,
  });

  card(s, 0.3, 3.85, 4.4, 1.55, C.deepBlue);
  s.addText("Capacidades principales", { x: 0.55, y: 3.9, w: 4.0, h: 0.4, fontSize: 14, bold: true, color: C.deepBlue, fontFace: "Calibri", margin: 0 });
  s.addText([
    { text: "Consultas sobre ciudades y países (seguridad, coste, salud…)", options: { bullet: true, breakLine: true } },
    { text: "Comparativas: \"Russia vs China healthcare\"", options: { bullet: true, breakLine: true } },
    { text: "Recomendaciones personalizadas por tema y presupuesto", options: { bullet: true, breakLine: true } },
    { text: "Rankings: top 10 safest countries", options: { bullet: true } },
  ], { x: 0.55, y: 4.28, w: 3.9, h: 1.1, fontSize: 10.5, color: C.textDark, fontFace: "Calibri", margin: 0 });

  s.addText("Stack Tecnológico", { x: 5.0, y: 1.28, w: 4.6, h: 0.35, fontSize: 13, bold: true, color: C.textMid, fontFace: "Calibri", margin: 0 });

  const stack = [
    { icon: "🐍", label: "Python + Flask", desc: "Microservicio chatbot NLP" },
    { icon: "🐘", label: "PHP + CodeIgniter 4", desc: "Frontend web MVC" },
    { icon: "🌐", label: "Three.js", desc: "Globo 3D interactivo" },
    { icon: "🐬", label: "MySQL 8.4", desc: "Persistencia de datos" },
    { icon: "🐳", label: "Docker Compose", desc: "Despliegue multi-servicio" },
  ];
...
// ═══════════════════════════════════════════════════════════════════════════════
// DIAPOSITIVA 3 — Representación del Conocimiento
// ═══════════════════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  sectionHeader(s, "Representación del Conocimiento", "Fuentes de datos, taxonomías y escalas de interpretación");

  s.addText("Fuentes de Datos (CSV)", { x: 0.3, y: 1.3, w: 4.3, h: 0.35, fontSize: 13, bold: true, color: C.deepBlue, fontFace: "Calibri", margin: 0 });

  const datasets = [
    { name: "Quality of Life Index", desc: "195 países · 7 índices" },
    { name: "Cost of Living Index", desc: "Índice de coste y alquiler" },
    { name: "Health Care Index", desc: "Calidad sanitaria global" },
    { name: "Crime Index", desc: "Seguridad por país" },
    { name: "Travel Cities Dataset", desc: "4.416 ciudades · 9 categorías" },
  ];
  ...
  s.addText("Taxonomías de Conocimiento", { x: 4.9, y: 1.3, w: 4.9, h: 0.35, fontSize: 13, bold: true, color: C.deepBlue, fontFace: "Calibri", margin: 0 });

  card(s, 4.9, 1.7, 4.8, 1.7, C.teal);
  s.addText("COUNTRY_METRICS — 5 dimensiones", { x: 5.15, y: 1.75, w: 4.4, h: 0.35, fontSize: 12, bold: true, color: C.teal, fontFace: "Calibri", margin: 0 });
  s.addText([
    { text: "safety · cost · healthcare · pollution · quality", options: { breakLine: true } },
    { text: "Cada dimensión incluye: palabras clave, columna CSV, orden mejor/peor", options: { italic: true } },
  ], { x: 5.15, y: 2.1, w: 4.4, h: 0.75, fontSize: 10.5, color: C.textDark, fontFace: "Calibri", margin: 0 });
  s.addText("Ejemplo: safety → keywords [\"safe\",\"crime\"] → Safety Index (↑ mejor)", {
  ...
  s.addText("CITY_TOPICS — 9 categorías de viaje", { x: 5.15, y: 3.6, w: 4.4, h: 0.35, fontSize: 12, bold: true, color: C.teal, fontFace: "Calibri", margin: 0 });
...
{
  const s = pres.addSlide();
  sectionHeader(s, "Motor de Inferencia", "Arquitectura de reglas y árbol de decisión");

  const steps = [
    { n: "1", label: "Normalización", detail: "lowercase, regex, strip", color: C.mint },
    { n: "2", label: "Tokenización NLP", detail: "NLTK: POS tag + lema", color: C.teal },
    { n: "3", label: "Reconoc. Entidades", detail: "find_mentions(): país/ciudad", color: C.deepBlue },
    { n: "4", label: "Aplicación Reglas", detail: "apply_rules(): 8 reglas priorizadas", color: "8B5CF6" },
    { n: "5", label: "Generación Respuesta", detail: "responses.py → JSON", color: "065A82" },
  ];
  ...
  s.addText("Árbol de decisión — apply_rules()", { x: 0.3, y: 2.55, w: 9.4, h: 0.35, fontSize: 13, bold: true, color: C.deepBlue, fontFace: "Calibri", margin: 0 });

  const rules = [
    { cond: "2+ países + (vs / compare / than)",       act: "compare_countries()",          col: C.mint },
    { cond: "2+ ciudades + comparación",               act: "compare_cities()",             col: C.teal },
    { cond: "recommend / suggest / where + go",        act: "recommend_cities()",           col: C.deepBlue },
    { cond: "weather + lugar",                         act: "show_weather() API",           col: "0284C7" },
    { cond: "capital / population / currency + país",  act: "show_country_facts()",         col: "7C3AED" },
    { cond: "país mencionado",                         act: "show_country_metric/summary()", col: "065A82" },
    { cond: "ciudad mencionada",                       act: "show_city_info()",             col: "0891B2" },
    { cond: "best / top / rank + lugar",               act: "show_best_cities/countries()", col: "059669" },
  ];
...
// ═══════════════════════════════════════════════════════════════════════════════
// DIAPOSITIVA 5 — Detalles de Implementación
// ═══════════════════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  sectionHeader(s, "Detalles de Implementación", "NLP, APIs externas y arquitectura de servicios");
  ...
  const nlpSteps = [
    ...
    { step: "WordNetLemmatizer()", detail: "Sustantivos → pos='n'  Verbos → pos='v'" },
  ...
  s.addText("APIs Externas", { x: 4.95, y: 1.3, w: 4.8, h: 0.35, fontSize: 13, bold: true, color: C.deepBlue, fontFace: "Calibri", margin: 0 });
  ...
  s.addText("restcountries.com/v3.1/name/{country}\nDevuelve: capital · population · languages · currencies", {
  ...
  s.addText("Geocoding → Coordenadas GPS\nForecast → temperatura · viento · código meteorológico", {
    x: 5.2, y: 3.68, w: 4.4, h: 0.55, fontSize: 10, color: C.textDark, fontFace: "Calibri", margin: 0,
  });
  s.addText("→ show_weather()  ·  19 condiciones meteorológicas", { x: 5.2, y: 4.22, w: 4.4, h: 0.42, fontSize: 10, color: C.textLight, fontFace: "Calibri", italic: true, margin: 0 });

  s.addShape(pres.shapes.RECTANGLE, { x: 0.3, y: 5.0, w: 9.45, h: 0.45, fill: { color: C.darkBg }, shadow: makeShadow() });
  s.addText("Flujo:  Browser → Nginx → PHP Controller → shell_exec(python3) → Flask /chat → JSON", {
    x: 0.45, y: 5.02, w: 9.2, h: 0.4, fontSize: 10, color: C.mint, fontFace: "Calibri", margin: 0,
  });
}

// ═══════════════════════════════════════════════════════════════════════════════
// DIAPOSITIVA 6 — Testing y Evaluación
// ═══════════════════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  sectionHeader(s, "Testing y Métodos de Evaluación", "PHPUnit, integración y evaluación de respuestas");

  s.addText("Estrategia de Testing", { x: 0.3, y: 1.3, w: 4.4, h: 0.35, fontSize: 13, bold: true, color: C.deepBlue, fontFace: "Calibri", margin: 0 });

  const tests = [
    { icon: "🧪", title: "PHPUnit 10.5+", body: "Framework de tests PHP\nDirectorio: /www/tests/\nCobertura HTML generada automáticamente" },
    { icon: "🔗", title: "Integración Docker", body: "PHP 8.4-fpm + XDebug activado\nMySQL 8.4 con seeding\nBase de datos de test aislada" },
  ...
  s.addText("Casos de Prueba representativos", { x: 4.95, y: 1.3, w: 4.8, h: 0.35, fontSize: 13, bold: true, color: C.deepBlue, fontFace: "Calibri", margin: 0 });

  const cases = [
    { query: "Is Japan safe?",                 type: "Consulta métrica",    ok: true },
    { query: "Russia vs China healthcare",     type: "Comparación",         ok: true },
    { query: "Best city for food under $50",   type: "Recomendación+budget",ok: true },
    { query: "What is the capital of France?", type: "Facts API",           ok: true },
    { query: "weather in Tokyo",               type: "API meteorología",    ok: true },
    { query: "Top 10 safest countries",        type: "Ranking",             ok: true },
    { query: "Tell me something random",       type: "Fuera de alcance",    ok: false },
    { query: "How do I get to Madrid?",        type: "No soportado",        ok: false },
  ];
  ...
  s.addText("✓ Cubierto    ~ Fuera de alcance (muestra texto de ayuda)", {
    x: 4.95, y: 5.35, w: 4.8, h: 0.22, fontSize: 9, color: C.textLight, fontFace: "Calibri", italic: true, margin: 0,
  });
}

// ═══════════════════════════════════════════════════════════════════════════════
// DIAPOSITIVA 7 — Demo del Sistema
// ═══════════════════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  sectionHeader(s, "Demo del Sistema", "Ejemplos de interacción y visualización 3D");
  ...
  const msgs = [
    ...
    { who: "bot",  text: "🏆 Top: Barcelona (food: 4.6/5 · nightlife: 4.5/5)\nAlso great: Bangkok, Tokyo, New York City." },
  ];
  ...
  s.addText("🌐 Visualización 3D Globe (Three.js)", { x: 4.85, y: 1.32, w: 4.85, h: 0.36, fontSize: 11, bold: true, color: C.white, fontFace: "Calibri", margin: 0 });
  ...
  const features = [
    "Drag & zoom interactivo",
    "Marcadores animados por respuesta",
    "Panel de ranking con 🥇🥈🥉",
    "195 capitales mapeadas",
  ];
  features.forEach((f, i) => {
    s.addShape(pres.shapes.OVAL, { x: 4.88, y: 4.65, w: 0.12, h: 0.12, fill: { color: C.mint } });
...
// ═══════════════════════════════════════════════════════════════════════════════
{
  ...
  s.addText("Conclusiones", { x: 0.5, y: 0.25, w: 6, h: 0.6, fontSize: 32, bold: true, color: C.white, fontFace: "Calibri", charSpacing: 3, margin: 0 });

  const conclusions = [
    { icon: "✅", text: "Sistema KBS completo: pipeline NLP → motor de reglas → datos CSV → respuesta natural" },
    { icon: "🌍", text: "Cobertura global: 195 países y 4.416+ ciudades con 5 métricas y 9 categorías" },
    { icon: "🔌", text: "Integración de APIs externas (REST Countries, Open-Meteo) para datos en tiempo real" },
    { icon: "🎨", text: "Experiencia de usuario diferenciada: globo 3D interactivo + chat conversacional" },
    { icon: "🧩", text: "Arquitectura modular Docker: escalable y fácil de desplegar" },
  ];
...
// ── Guardar ───────────────────────────────────────────────────────────────────
pres.writeFile({ fileName: "Globie_Presentacion_SBC_ES.pptx" })
  .then(() => console.log("✅  Globie_Presentacion_SBC_ES.pptx creada"))
